  ...
  const getPosts = async () => {
    try {
      const response = await apiClient.get(
        "https://comp1640.pythonanywhere.com/get_my_posts/pending"
        ...
        const updatedPosts = posts.map(
          (post) =>
            post._id === postId
              ? {
                  ...post,
                  comments_list: update_comment_list, // Cập nhật comments_list với danh sách bình luận mới
                }
              : post // Trả về post không thay đổi nếu không phải bài viết cần cập nhật
        );
